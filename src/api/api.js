...
export const fetchTranscription = async (date, callId) => {
    let finalDate = date;
    ...
    if (!isDate(finalDate) || isNaN(finalDate)) {
      throw new Error("Invalid selectedDate provided to fetchTranscription");
    }
  
    const formattedDate = format(finalDate, 'yyyy-MM-dd');
  
    const url = `${process.env.REACT_APP_CALL_API}?date=${formattedDate}&call_id=${callId}`;  
    ...
    const callDetails = Array.isArray(data) 
      ? data.find(item => item.id === callId) 
      : data;
  
    if (!callDetails) {
      console.warn(`Call ID ${callId} not found in API response`);
      return {
    ...
    return {
      transcription: callDetails.transcription || "No transcription available.",
      transcriptionClean: callDetails.transcription_clean || null,
    };
};
